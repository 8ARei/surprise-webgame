...
function Level(config, isIntro) {
	...
	self.playbackFrame = function (frameIndex) {
		...
		if (self.keyCollected && !lastCollected && STAGE == 3) {
			createjs.Sound.play('pickup', { volume: 0.05 })
		}
...
function Clock(countdown, level) {
	...
	self.update = function () {
		...
		if (self.frame >= 30) {
			createjs.Sound.play('error', { volume: 0.05 })
			reset()
		}
	}
...
function DoorKey(config, level) {
	...
	self.draw = function (ctx) {
		...
		ctx.scale(0.7, 0.7)
		ctx.drawImage(images.key, -23, -14, 47, 32)
		ctx.restore()
	}
...
function Door(config, level) {
		...
		if (level.keyCollected) {
			...
			if (distance < 6) {
				if (level.isIntro) {
					...
					createjs.Sound.play('ding', { volume: 0.05 })
	...
	self.draw = function (ctx) {
		...
		var dx = -42 //-2288 //-34
		var dy = -95 //-2265 //-91
		ctx.drawImage(images.door, sx, sy, sw, sh, dx, dy, 75, 98)
		ctx.restore()
	}
	self.drawShadow = function (ctx) {
		ctx.save()
...
function Peep(config, level) {
	...
	self.draw = function (ctx) {
		...
		if (self.frame == 4 || self.frame == 9) {
			createjs.Sound.play('step', { volume: 0.05 })
		}
		...
		ctx.drawImage(images.peep, -42.5, -100, 85, 130)
		ctx.restore()
	}
...
window.onload = function () {
	...
	addSound('pickup', 'audio/pickup.mp3')
		...
		function update() {
			...
			if (STAGE == 3 && !window.HAS_PLAYED_JAZZ) {
				if (STAGE == 3 && CURRENT_LEVEL == 1) {
					var framesLeft =
						rewindLevel.frames.length - rewindFrame + levelObjects[2].frames.length
					if (framesLeft < 135) {
						window.HAS_PLAYED_JAZZ = true
						createjs.Sound.play('jazz', { volume: 0.05 })
					}
				}
			}
		}
...
function next() {
	...
	if (CURRENT_LEVEL < LEVEL_CONFIG.length) {
		...
		if (extraTime < 0) {
			createjs.Sound.play('rewind', { volume: 0.1 })
		} else {
			createjs.Sound.play('rewind', { volume: 0.1 }, 'none', 0, extraTime)
		}
...
function iHeartYou() {
	...
	document.getElementById('screen_two').style.background = '#bce0fa'

	var can_cont_text = document.getElementById('canvas_container_text')

	var vtext = document.getElementById('HBD_text')
	...
	if (window.location.hash) {
		...
		vtext.textContent = 'I love you so much baby Angela ♡'
	}
